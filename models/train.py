...
if __name__ == "__main__":
    ...
    print("1. Engenharia de Features...")
    CURRENT_YEAR = 2025
    df['log_price'] = np.log1p(df['price_clean'])
    df['car_age'] = CURRENT_YEAR - df['ano_limpo']
    df.loc[df['car_age'] <= 0, 'car_age'] = 0.5
    ...
    cols_to_drop = ['log_price', 'price_clean', 'url', 'title_list', 'description', 
                    'color_list', 'modelo', 'city_clean', 'neighborhood_clean', 'ano_limpo']
    valid_cols_to_drop = [col for col in cols_to_drop if col in df.columns]
    X_full = df.drop(columns=valid_cols_to_drop)
    
    print("2. Construindo Preprocessor...")
    numeric_features = X_full.select_dtypes(include=['float64', 'int64']).columns.tolist()
    categorical_features = X_full.select_dtypes(include=['object']).columns.tolist()
    boolean_features = X_full.select_dtypes(include=['bool']).columns.tolist()
    ...
    print("3. Construindo Pipeline Final...")
    model_pipeline = Pipeline(steps=[
        ('preprocessor', preprocessor),
        ('model', XGBRegressor(**V4_GOLDEN_PARAMS))
    ])
    
    print(f"4. Treinando modelo em {len(X_full)} amostras...")
    model_pipeline.fit(X_full, y_full)
    print("Treinamento concluído.")
    
    output_path = os.path.join(project_root, 'models', 'price_predictor_v1.pkl')
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    
    print(f"5. Salvando modelo em: {output_path}")
    joblib.dump(model_pipeline, output_path)
    print("--- Modelo salvo com sucesso! ---")
    print("--- FIM DA SEMANA 3 ---")